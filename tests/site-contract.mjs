...
const soeHeroMatch = soe.match(
  /<section\b(?=[^>]*\bclass=["'][^"']*\bhero\b[^"']*["'])[^>]*>[\s\S]*?<\/section>/i,
);
assert.ok(soeHeroMatch, 'SOE page must contain a hero section');
const soeHero = soeHeroMatch[0];
assert.match(soeHero, /class=["'][^"']*\btsinghua-identity\b[^"']*["']/i, 'SOE hero must use a left-column Tsinghua identity block');
assert.match(soeHero, /class=["'][^"']*\bhero-body\b[^"']*["']/i, 'SOE hero must contain a hero body');
const soeHeroCopy = findElementByClass(soeHero, 'hero-copy');
assert.ok(soeHeroCopy, 'SOE hero must contain a copy column');
assert.match(soeHeroCopy, /class=["'][^"']*\btsinghua-identity\b[^"']*["']/i, 'SOE Tsinghua identity must stay inside the copy column');
...
assert.match(soeHero, /class=["'][^"']*tsinghua-degree[^"']*["'][^>]*>电子信息（085400）<\/p>/i, 'SOE identity must contain the exact degree');
assert.match(soeHero, /<img\b[^>]*class=["'][^"']*tsinghua-mark[^"']*["'][^>]*src=["']\/assets\/images\/tsinghua-seal\.jpg["'][^>]*width=["']600["'][^>]*height=["']598["']/i, 'SOE Tsinghua identity must use the supplied seal dimensions');
...
assert.match(files['assets/css/soe.css'], /\.tsinghua-identity\b/i, 'SOE CSS must style the Tsinghua identity block');
assert.match(files['assets/css/soe.css'], /\.hero-body\b/i, 'SOE CSS must style the hero body');
assert.match(files['assets/css/soe.css'], /\.hero-email\b/i, 'SOE CSS must style the hero email');
...
assert.ok(!soe.includes('2025.06.18 转正'), 'SOE page must not retain the party membership confirmation date');
